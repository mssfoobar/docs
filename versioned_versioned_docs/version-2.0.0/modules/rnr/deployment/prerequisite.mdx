...
The following scripts are an example of how to create the user for the `gis` database.

```sql
-- Create the user with a secure password
CREATE USER rnr_playback_user WITH PASSWORD 'your_secure_password_here';
```
...
```sql
ALTER DATABASE gis OWNER TO rnr_playback_user;
```
...
SELECT 'CREATE DATABASE gis'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'gis')\gexec
