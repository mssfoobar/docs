...
type AuthResult = AuthResultSuccess | AuthResultFailed
type AuthResultSuccess = {
	success: true,
	claims: AuthClaims
	access_token: uuid
}
type AuthResultFailed = {
	success: false
}
```
...
- The user claims as provided by Keycloak.
- An access token that can be used to decode and retrieve user information or to authenticate requests to other APIs (e.g., constructing headers).
  
### AuthResultFailed
AuthResultFail indicates a failed authentication and contains:
	- A boolean success field that is always set to false.

This structure allows you to handle both successful and failed authentication outcomes in a clear and type-safe manner.
