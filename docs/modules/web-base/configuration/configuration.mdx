...
<details>
	...
	<div>
		...
		set. Note that when developing using `localhost` as the domain, this **must** be left blank for Chromium
		browsers.
    </div>
...
We use [changesets](/docs/contributing/development/source-control.mdx#changesets) to help track changes and manage
...
[`conventional commits`](/docs/contributing/development/source-control.mdx#conventional-commits). You can remove this
configuration but we recommend you also govern how your commit messages are written to keep your messages neat,
