...
This guide assumes that you will be integrating the IAN components with the web-base. If you have yet to set up the
web-base, you can do so [here](/docs/modules/web-base/overview/overview.md).
...
Include the following code block below above the `let tenant: Tenant | undefined = $state(undefined);` line:
