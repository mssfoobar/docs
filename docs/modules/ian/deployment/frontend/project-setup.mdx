...
## Set up the IAN Environment
To set up the environment, follow the instructions in the `env.example` file to fill in the necessary environment variables. The `IAM_CLIENT_SECRET` value can be obtained by following the instructions in the [Authentication - Browser Login](/docs/modules/iams/development/authentication/browser-login.mdx#configuration) guide.

:::note
If you're using the [dev-container](/docs/modules/get-started/local-development.mdx) to deploy the IAMS module, the client name `ian` is already available in the client list. You can use this client's secret, eliminating the need to set up a new one.
:::
...
:::info
For more details, refer to this [guide](/docs/modules/iams/configuration/manual-keycloak-realm-setup/setup-system-admin-account.md)
:::
...
![Select Tenants Menu](/img/modules/ian/web/tenants-menu.png)

4. Click on the tenant you want.

![Select Tenant](/img/modules/ian/web/tenant-list.png)

5. Click on `Select Users` button.

![Select User Button](/img/modules/ian/web/select-user-button.png)

6. Check box the user accounts you want to add to tenant on the left column.

![Select User](/img/modules/ian/web/select-user.png)

7. Click on `Confirm` to complete the process.

![Confirm Button](/img/modules/ian/web/confirm-button.png)