...
To stop the module, simply execute the command in step 3 with the `up -d` replace with `down`.

For example, to stop the running IAN module, execute the following:

```bash
podman compose --env-file .env -f ian/compose.yml -f ./override.yml down
```
