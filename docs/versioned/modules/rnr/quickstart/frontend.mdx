...
const systemTimeTopic = PUBLIC_SYSTEM_TIME_TOPIC_NAME;
const systemTimeTenantId = PUBLIC_SYSTEM_TIME_TENANT_ID;
```

Include the `<PlaybackContainer>` component within the `<AuthProvider>` component.
