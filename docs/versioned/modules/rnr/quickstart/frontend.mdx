...
In the app.d.ts file, add these in the global namespace:
...
This component **dispatches custom events globally using `document.dispatchEvent`** on status updates. Other modules can listen to these events to react to changes without coupling tightly to this component’s logic.

---

#### Event Name

The event name is the current status of the running session. Refer to [this documentation](../development/backend/session-state.mdx) for a complete list of statuses.

#### Listening for the Event

To listen for these events in a Svelte module:
...
<script lang="ts">
    ...
    function handleReplayInit() {
        // Your logic here
        console.log('REPLAY_INIT_OK event received');
    }

    onMount(() => {
        // Add the event listener when the component is mounted
        document.addEventListener("REPLAY_INIT_OK", handleReplayInit);

        // Clean up the event listener when the component is destroyed
        return () => {
            document.removeEventListener("REPLAY_INIT_OK", handleReplayInit);
        };
    });
</script>
```
