...
:::important
Database user in application.properties configuration **MUST** have these permissions: `REPLICATION`, `LOGIN`,
`CREATE` and `SELECT`. Otherwise, Debezium will fail to create a replication slot in the Postgres server.
:::

```text title="application.properties"
debezium.source.slot.name=recording_connector
...
| Name | Example | Description |
|---|---|---|
| debezium.source.slot.name | `recording_connector` | The name of the PostgreSQL logical replication slot. |
