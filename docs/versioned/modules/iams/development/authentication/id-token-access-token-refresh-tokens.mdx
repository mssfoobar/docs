...
:::danger
As the `userinfo` endpoint is an OIDC endpoint, it requires the "openid" scope to be one of the access token's claims. When attempting to use the `userinfo` endpoint to
validate tokens associated with `Service Accounts`, please remember to include the "openid" scope when requesting for the token.
