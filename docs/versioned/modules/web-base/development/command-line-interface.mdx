...
ensure you have read the [Modlet Development](/stable/contributing/development/web-module-development) section
before continuing.
:::

-   **Package Name**: This corresponds to your module name, and the folder you place all your module files in
    ([for example see "bar" here](/docs/stable/contributing/development/web-module-development#cli-tool-packaging-requirements))
-   **Project Name**: The name of your project - this is always `aoh` when performing development for AGIL Ops Hub
    modules ([for example see "foo" here](/docs/stable/contributing/development/web-module-development#cli-tool-packaging-requirements)).
...
The `publish` command is designed to run with the `pack` command. After running `pack`, you can run `cli publish` to
