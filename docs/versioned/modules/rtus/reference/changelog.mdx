...
- 5f13e56: Change keepalive from comment to event for client side to detect connection issue
  - **Breaking Change**: The SSE keepalive message has been changed from a comment (`: keepalive`) to a proper SSE event with name `keepalive` and data `ping`
  - This change allows clients to properly detect connection issues and handle reconnection logic
  - Clients should update their SSE event handlers to listen for the `keepalive` event instead of ignoring comments
