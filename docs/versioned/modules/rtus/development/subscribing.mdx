...
For **Map** and **JSON Map**, it is possible to ask RTUS to send all key-value pairs available in the Map during initial connection. To do that, simply add `init=true` as query parameter in the URL. 
For example:
...
As of rtus-seh v1.2.1, the initial value will be send as **Added** named event. 

Prior to version 1.2.1, the initial value were sent as standard
message without event field.
...
Prior to version 1.1.3 (rtus-pms) and 1.2.0 (rtus-seh), keepalive messages were sent as comments and were not detectable by client-side JavaScript. If you are using the latest version, update to latest SSE client library to listen for the `keepalive` event for better connection monitoring.
