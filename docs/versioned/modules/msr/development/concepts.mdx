...
| Feature | Change Data Capture (CDC) | Event Sourcing (ES) |
|---------|---------------------------|---------------------|
| **Architectural Impact** | Minimally invasive; retrofits onto existing systems | Pervasive; requires complete system redesign |
| **Implementation Complexity** | Relatively low; uses mature database features | High; requires event stores, projections, versioning |
| **Source of Truth** | Primary database transaction log | Append-only event log |
| **Suitability for Retrofitting** | Excellent | Poor; requires fundamental re-architecture |
...
MSR uses **TimescaleDB Continuous Aggregates** instead of standard PostgreSQL materialized views for superior performance:

| Feature | Standard Materialized View | TimescaleDB Continuous Aggregate |
|---------|---------------------------|----------------------------------|
| **Refresh Mechanism** | Manual, full rebuild | Automatic, incremental updates |
| **Query Freshness** | Stale until refresh | Real-time with automatic combination |
| **Performance Impact** | High periodic load | Low, consistent background load |
| **Data Retention** | Poor integration | Excellent integration with retention policies |
