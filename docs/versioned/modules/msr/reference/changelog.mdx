...
-   fix: include boundary events in state reconstruction queries
-   Fixed a bug where events occurring exactly at snapshot boundary times were excluded from state initialization.
    Changed `event_timestamp > cutoff` to `event_timestamp >= cutoff` in both Go repository queries and SQL function to
    ensure complete temporal correctness during replay operations.
