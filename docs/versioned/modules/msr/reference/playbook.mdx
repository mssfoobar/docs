...
**Applies to**: MSR deployments created before 2025-11-04

**Issue**: The `entity_last_states` continuous aggregate uses `time_bucket('1 day', event_timestamp)` which labels daily buckets with their **start time** (e.g., `2025-01-15 00:00:00` for events from Jan 15). However, `LAST(entity_state, event_timestamp)` stores the **final state from the entire day** (potentially 23:59:59). The application code incorrectly treats this as "state at the beginning of the day", causing **future data leakage** during replay operations.

**Impact**: Replay sessions may include entity states from **after** the requested replay timestamp, violating temporal correctness.

**Solution**: Add a `+ INTERVAL '1 day'` offset to `snapshot_time` so buckets are labeled with their **end time**, matching application expectations.
...
If you need to rollback (e.g., unexpected issues):

```sql
-- Restore original definition (without +1 day offset)
DROP MATERIALIZED VIEW msr.entity_last_states CASCADE;
...
SELECT entity_id, table_name,
       LAST(entity_state, event_timestamp) AS entity_state,
       LAST(op, event_timestamp) AS op,
       time_bucket('1 day', event_timestamp) AS snapshot_time  -- Original: no offset
FROM msr.cdc_event
GROUP BY snapshot_time, entity_id, table_name;

-- Recreate policies (use your production values)
-- ... (same as Step 4 above, but may have original bug)
```
