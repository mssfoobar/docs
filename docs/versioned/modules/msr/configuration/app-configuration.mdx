...
| -------------------- | ------------- | ----------- |
| `PUBLIC_MSR_API_URL` | `http://msr.127.0.0.1.nip.io` | Base URL for MSR backend API endpoints |
| `PUBLIC_MSR_WS_URL` | `ws://msr.127.0.0.1.nip.io` | WebSocket URL for real-time communication (if needed) |
| `PUBLIC_MSR_POLL_WINDOW_MS` | `3000` | Default polling window in milliseconds for CDC data fetching |
| `PUBLIC_MSR_FRAME_RATE` | `30` | Default frame rate for replay rendering |
| `PUBLIC_MSR_MAX_BUFFER_SIZE` | `1000000` | Default maximum buffer size for change events |
| `PUBLIC_MSR_MIN_POLLING_INTERVAL` | `100` | Default minimum polling interval in milliseconds |
