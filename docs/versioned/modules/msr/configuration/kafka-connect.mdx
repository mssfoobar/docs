...
Never use hardcoded passwords in production environments. Always use environment variables or secure credential management systems to store and retrieve database passwords. The examples below use `${DATABASE_PASSWORD}` placeholders that should be replaced with actual environment variables or secure vault references.
:::
...
| `database.password` | Database password | `${DATABASE_PASSWORD}` |
...
| `connection.password` | Database password | `${DATABASE_PASSWORD}` |
