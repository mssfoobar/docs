...
Since we want our application to be cloud native, we need to have a good degree of infrastructure automation. We'll have
...
For example, someone found a typo in the web `develop` branch (this branch deploys to a preview server for us to
view changes made to the web server) - for a change to be made, a developer must use the `develop` branch as the base
and checkout a new branch with a new name (e.g. `fix/ambulance-chart-typo`). They then make the changes to fix this
typographical error, and open a pull request to contribute this change back to the `develop` branch. This pull
...
We have 4 categories of branches:

-   #### Production

    -   `main`

    The branch called `main` is reserved specifically for the most up to date code. This might map directly to a
...
-   #### Release
    ...
    Branches starting with "release" are reserved for code that is being prepared for release

-   #### Development

    -   `develop`

    The branch called `develop` is reserved for the latest development code. This will be the base for
...
-   #### Feature
    ...
    Developers are to work on branches with a naming scheme that starts with
    `feat`, `fix`, `release`, `refactor`, `chore` or `style`, followed by a slash, then a kebab case description. This
    is to help us quickly identify the purpose of each branch. Example branch name - `fix/ambulance-chart-typo`

### Changesets

We use [changesets](https://github.com/changesets/changesets) to managing versioning and changelogs for our
repositories.

### Code Merging (via Pull/Merge requests)

The expected workflow for merging code is as follows:

You've finished fixing the typographical errors in your `fix/ambulance-chart-typo` branch and wish to merge it back
to the develop branch - you've run `changeset` to record your changes and committed your code following our standards.

1. Create a pull request
2. Inform the team (if configured appropriately on GitHub, [CODEOWNERS](https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/about-code-owners)
   and other relevant members might be notified automatically via GitHub )
3. Discuss and make changes if required
4. Once approved, and all checks passs, use [Squash & Merge](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/incorporating-changes-from-a-pull-request/about-pull-request-merges#squash-and-merge-your-commits)
...
The actual rule we use for allowed types (the prefix of the commit message) can be found in the
[web-base's commitlint config file](https://github.com/mssfoobar/web-base/blob/develop/.commitlintrc.cts).
...
GitHub rulesets for each branch is configured to restrict pushes to specific branches without
a pull request with reviewers, as well as add branch naming patterns to follow, block force pushes etc. Refer to the
[`Web Base`'s rule settings](https://github.com/mssfoobar/web-base/settings/rules) for reference - you can export and
reuse these rules in your new repository for convenience.

2. `Husky`

[Husky](https://typicode.github.io/husky/) is a tool we use to control the quality of code and enforce commit standards. Before each commit, linting and
...
![image](/img/github-settings-keys.png)
